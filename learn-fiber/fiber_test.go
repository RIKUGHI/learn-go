...
func TestHttpRequest(t *testing.T) {
	...
	request.Header.Set("firstname", "Bambang")
	request.AddCookie(&http.Cookie{Name: "lastname", Value: "Cahyo"})
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Bambang Cahyo", string(bytes))
}
...
func TestRouteParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/users/bambang/orders/10", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Get Order 10 From User bambang", string(bytes))
}
...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=Bambang")
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Bambang", string(bytes))
}
...
func TestBodyParserJSON(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Bambang", "password":"rahasia", "name":"Bambang Cahyo"}`)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Register Success Bambang", string(bytes))
}
...
func TestBodyParserXML(t *testing.T) {
	...
	body := strings.NewReader(
		`<RegisterRequest>
			<username>Bambang</username>
			<password>rahasia</password>
			<name>Bambang Cahyo</name>
		</RegisterRequest>
		`)
	request := httptest.NewRequest("POST", "/register", body)
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"Bambang Cahyo","username":"Bambang"}`, string(bytes))
}
